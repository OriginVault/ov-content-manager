...
import { provisionSandboxCert, CertProviderConfig } from "./sslSigner.js";

export type SupportedTypeInfo = { format: string; extension: string };

export const C2PA_SUPPORTED_TYPES: Record<string, SupportedTypeInfo> = {
...
export class C2PA {
  private c2pa: any;
  private signer: any;
  private certConfig?: CertProviderConfig;
  private initializedWithSsl = false;

  constructor(signerOrConfig?: any | CertProviderConfig) {
    this.c2pa = createC2pa();
    if (signerOrConfig && isCertConfig(signerOrConfig)) {
      this.certConfig = signerOrConfig;
      this.signer = null;
    } else {
      this.signer = signerOrConfig || createTestSigner();
    }
  }

  private async ensureSigner(): Promise<any> {
    if (this.signer) return this.signer;
    if (this.certConfig && !this.initializedWithSsl) {
      const material = await provisionSandboxCert(this.certConfig);
      this.signer = {
        privateKey: material.privateKeyPem,
        certificate: material.certificatePem,
        algorithm: 'ES256'
      };
      this.initializedWithSsl = true;
      return this.signer;
    }
    // fallback
  ...
  getFileTypeInfo(mimeType: string): SupportedTypeInfo | null {
    return (C2PA_SUPPORTED_TYPES as Record<string, SupportedTypeInfo>)[mimeType] || null;
  }
  ...
  async generateManifest(
    ...
    try {
      ...
      if (!this.isSupported(mimeType)) {
        return { success: false, error: `File type ${mimeType} is not supported for C2PA manifest generation` };
      }

      const fileTypeInfo = this.getFileTypeInfo(mimeType);
      if (!fileTypeInfo) {
        return { success: false, error: `Unable to determine file type info for ${mimeType}` };
      }

      const manifest = new ManifestBuilder({
        title: options.title,
        format: fileTypeInfo.format,
        claim_generator: "OriginVault C2PA",
      ...
      return { success: true, manifest, manifestKey, manifestMnemonicId };
    } catch (error: any) {
  ...
  getCategorySpecificMetadata(category: string, options: C2PAManifestOptions): any {
    ...
    switch (category) {
      case "image":
        return { ...baseMetadata, "xmp:ImageWidth": "auto-detected", "xmp:ImageHeight": "auto-detected", "xmp:ColorSpace": "auto-detected", "xmp:ColorMode": "auto-detected" };
      case "video":
        return { ...baseMetadata, "xmp:VideoWidth": "auto-detected", "xmp:VideoHeight": "auto-detected", "xmp:VideoFrameRate": "auto-detected", "xmp:VideoDuration": "auto-detected", "xmp:VideoCodec": "auto-detected" };
      case "audio":
        return { ...baseMetadata, "xmp:AudioSampleRate": "auto-detected", "xmp:AudioChannels": "auto-detected", "xmp:AudioDuration": "auto-detected", "xmp:AudioCodec": "auto-detected" };
      case "3d":
        return { ...baseMetadata, "xmp:ModelType": "3D", "xmp:ModelFormat": options.mimeType, "xmp:ModelVersion": "1.0" };
      case "document":
        return { ...baseMetadata, "xmp:DocumentType": "Document", "xmp:DocumentFormat": options.mimeType, "xmp:PageCount": "auto-detected" };
      case "font":
        return { ...baseMetadata, "xmp:FontType": "Font", "xmp:FontFormat": options.mimeType, "xmp:FontFamily": "auto-detected" };
      default:
        return baseMetadata;
    }
  }
  ...
  async signFile(fileBuffer: Buffer, mimeType: string, manifest: any): Promise<Buffer | null> {
    try {
      ...
      const signedBuffer = await this.c2pa.sign({ asset, signer, manifest });
  ...
  async readManifest(fileBuffer: Buffer, mimeType: string): Promise<any> {
    try {
      return await this.c2pa.read({ buffer: fileBuffer, mimeType });
    } catch {
      return null;
    }
  }
...
export function createC2paClient(signerOrConfig?: any | CertProviderConfig): C2PA {
  return new C2PA(signerOrConfig);
}

function isCertConfig(v: any): v is CertProviderConfig {
  return v && typeof v === 'object' && 'apiUrl' in v && 'apiKey' in v && 'certificateProfileId' in v && 'conformingProductId' in v;
}

// Export high-level signing API
