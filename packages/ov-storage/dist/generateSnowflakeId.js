...
export function snowflakeToMnemonic(snowflakeID) {
    ...
    if (!chunks)
        throw new Error("Failed to split Snowflake ID");
...
export function mnemonicToSnowflake(mnemonic) {
    ...
    const binary = words
        ...
        if (originalValue < 0)
            originalValue += englishWordlist.length;
...
function deriveWorkerIdFromDid(did) {
...
export function generateSnowflakeId(did) {
