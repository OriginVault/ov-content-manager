 ...
 */
router.get("/quota", requireAuth, bucketLimiter, async (req, res) => {
    try {
        const auth = req.auth;
        if (!auth || !auth.sub) {
 ...
 */
router.get("/stats", requireAuth, bucketLimiter, async (req, res) => {
    try {
        const auth = req.auth;
        if (!auth || !auth.sub) {
            res.status(401).json({ error: "User not authenticated" });
 ...
 */
router.post("/cleanup", requireAuth, bucketLimiter, async (req, res) => {
    try {
        const auth = req.auth;
        if (!auth || !auth.sub) {
            res.status(401).json({ error: "User not authenticated" });
 ...
 */
router.get("/supported-types", bucketLimiter, async (req, res) => {
    try {
 ...
 */
router.get("/check-type/:mimeType", bucketLimiter, async (req, res) => {
    try {
 ...
 */
router.post("/clear-cache", requireAuth, bucketLimiter, async (req, res) => {
    try {
        const auth = req.auth;
        if (!auth || !auth.sub) {
            res.status(401).json({ error: "User not authenticated" });
