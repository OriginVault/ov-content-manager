...
async function canAccessStorageDid(userId, storageDid) {
    try {
...
router.get("/b/:did/presign", storageLimiter, requireAuth, async (req, res) => {
    try {
        const { did } = req.params;
        ...
        const expiresNum = Math.min(parseInt(expires) || 900, 3600); // Max 1 hour
        let presignedUrl;
        if (op === "get") {
...
router.post("/b/:did/presign", storageLimiter, requireAuth, async (req, res) => {
    try {
        const { did } = req.params;
        ...
        const presignedUrl = await minioClient.presignedPutObject(config.minio.bucket, normalizedKey, expiresNum);
        res.status(200).json({
            presignedUrl,
            expires: expiresNum,
            key: normalizedKey,
...
// Delete object
router.delete("/b/:did/objects/delete/:key", storageLimiter, requireAuth, async (req, res) => {
    try {
