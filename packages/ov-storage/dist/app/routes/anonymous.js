...
router.post("/upload-anonymous", uploadLimiter, upload.single("file"), async (req, res) => {
    try {
        const file = req.file;
        if (!file) {
        ...
        // Verify hCaptcha
        const hcaptchaToken = req.headers["x-hcaptcha-token"];
        if (!hcaptchaToken) {
        ...
        try {
            ...
            if (anonymousBucketDid) {
                ...
                const pouResponse = await storageService.createProofOfUpload(anonymousBucketDid, snowflake, mnemonicId, contentHash, sanitizedName, file.size, file.mimetype, uploaderIpHash, manifestKey || undefined, manifestMnemonicId || undefined);
...
router.get("/files/by-mnemonic/:mnemonic", async (req, res) => {
    try {
        ...
        for await (const obj of objects) {
            ...
            const presignedUrl = await minioClient.presignedGetObject(config.minio.bucket, obj.name, 900 // 15 minutes
            );
            files.push({
...
router.get("/manifests/by-mnemonic/:mnemonic", async (req, res) => {
    try {
        const { mnemonic } = req.params;
...
router.get("/proof-of-upload/:did", async (req, res) => {
    try {
