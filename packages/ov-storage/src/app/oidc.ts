...
async function discover(): Promise<DiscoveryDoc> {
  ...
  if (!issuer || !jwks_uri) throw new Error("Missing issuer or jwks_uri after discovery");
...
export type ValidatedToken = {
  payload: JWTPayload & { sub?: string; aud?: string | string[]; azp?: string; client_id?: string; scope?: string };
};

export async function validateBearerToken(authHeader: string): Promise<ValidatedToken> {
  ...
  if (allowedClientIds.length > 0) {
    const presented = payload.azp || (payload as any).client_id;
