// C2PA functionality moved into @originvault/ov-c2pa
import { getPrimaryDID, signVC, createResource, parentAgent, parentStore } from "@originvault/ov-id-sdk";
import logger from "./logger.js";
import * as Minio from "minio";
import { minioClient as rawMinioClient, BUCKET, streamToBuffer, ensureBucket } from "./app/minio.js";
const minioClient: Minio.Client = rawMinioClient as unknown as Minio.Client;
...
const hashFile = async (fileBuffer: ArrayBuffer) => {
...
const getFragmentedFile = async (fileBuffer: ArrayBuffer) => {
  ...
  const fragments: { blob: Blob, name: string }[] = [];
...
const getColorFragments = async (fileBuffer: ArrayBuffer, fragments: { blob: Blob, name: string }[]) => {
  const colorFragments: string[] = [];
...
// SeaweedFS S3-compatible client is provided by ./app/minio.js

const app = express();
// Ensure default bucket exists on startup (SeaweedFS S3)
...
// BUCKET is provided by ./app/minio.js

// In-memory store for presigned URLs
const urlStore: { [key: string]: { url: string; expiresAt: number; expectedContentHash: string; expectedSoftPerceptualHash: string; expectedMediumPerceptualHash: string; expectedPrecisePerceptualHash: string } } = {};
...
// streamToBuffer is provided by ./app/minio.js
...
export async function createContentRegistration(record: any, mnemonicId: string) {
  ...
  try {
    ...
    logger.error("Error creating content registration: " + (error as Error).message);
    return null;
  }
}

// 🆕 NEW: Write identity + file map to SeaweedFS S3 in one place.
async function writeIdentityAndMap({
  contentHash,
  softPerceptualHash,
  ...
  await minioClient.putObject(
    BUCKET,
    identityPath,
    Buffer.from(JSON.stringify(identityData)),
    undefined,
 ...
 */
async function computePerceptualHashes(buffer: Buffer): Promise<{
  softPerceptualHash: string;
  mediumPerceptualHash: string;
  precisePerceptualHash: string;
}> {
 ...
 */
function computeCarbonFootprint(byteLength: number) {
...
app.get("/identity/:contentHash", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    logger.error("Error fetching identity: " + (error as Error).message);
...
app.post("/check-file-uploaded", async (req: Request, res: Response): Promise<void> => {
...
app.post("/request-upload-url", uploadLimiter as any, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    let count = 0;
    for await (const _ of listStream) count++;
    if (count >= 10) {
...
app.post("/request-public-upload-url", uploadLimiter as any, async (req: Request, res: Response): Promise<void> => {
  const {
    fileName,
    userDID,
    username,
  ...
  try {
    ...
    logger.error("Error getting public presigned url: " + (error as Error).message);
    res.status(500).json({ error: "Presigned url retrieval failed" });
    return;
  }
});
...
app.post("/upload/:uri", uploadLimiter as any, upload.single("file"), async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const { softPerceptualHash, mediumPerceptualHash, precisePerceptualHash } =
      await computePerceptualHashes(fileBuffer);
    ...
    // 7) Actually store the file in SeaweedFS S3 using the presigned "entry.url"
    ...
    if (!uploadResponse.ok) {
      throw new Error("Failed to upload file to SeaweedFS via presigned URL");
    }
...
app.get("/user_upload_count/:userDID", async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error("Error counting user uploads: " + (error as Error).message);
    res.status(500).json({ error: "Failed to retrieve upload count" });
    return;
  }
});

app.get("/user_indexes/:userDID", async (req: Request, res: Response): Promise<void> => {
  const { userDID } = req.params;
  ...
  try {
    ...
    logger.error("Error listing user indexes: " + (error as Error).message);
...
app.get("/user_indexes/:username", async (req: Request, res: Response): Promise<void> => {
...
app.get("/public_files/:username", async (req: Request, res: Response): Promise<void> => {
  const { username } = req.params;
  ...
  try {
    ...
    logger.error("Error listing public files: " + (error as Error).message);
    res.status(500).json({ error: "Failed to retrieve public files" });
    return;
  }
});

app.delete("/delete_upload/:userDID/:fileId", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    logger.error("Error deleting upload: " + (error as Error).message);
    res.status(500).json({ error: "Deletion failed" });
    return;
  }
});

app.delete("/delete_manifest/:userDID/:manifestId", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    logger.error("Error deleting manifest: " + (error as Error).message);
    res.status(500).json({ error: "Manifest deletion failed" });
    return;
  }
});

app.delete("/delete_public_manifest/:username/:manifestId", async (req: Request, res: Response): Promise<void> => {
...
app.post("/sign", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const vc = await signVC(manifest.asSendable(), process.env.ORIGIN_VAULT_DID_PASSWORD || "")
      ...
      .catch((err: any) => {
        logger.error("Error signing VC: " + (err as Error).message);
      });
...
app.get("/health", async (req: Request, res: Response): Promise<void> => {
  try {
    await getPrimaryDID()
      .then((rep: string) => {
        res.json({ message: `C2PA Server is healthy and SeaweedFS is reachable. Primary DID: ${rep}` });
        return;
      })
      .catch((err: any) => {
        logger.error("Error fetching Primary DID: " + (err as Error).message);
        if (!res.headersSent) {
          res.status(200).json({
            message: "C2PA Server is healthy and SeaweedFS is reachable. Primary DID not found.",
          });
          return;
        }
      });
  } catch (error) {
    logger.error("Error fetching Primary DID: " + (error as Error).message);
    if (!res.headersSent) {
      res.status(500).json({ message: "C2PA Server is healthy, but SeaweedFS is not reachable" });
      return;
    }
  }
});
...
app.post("/create_bucket", async (req: Request, res: Response): Promise<void> => {
...
app.get("/bucket_exists", async (req: Request, res: Response): Promise<void> => {
  const { bucketName } = req.body;
...
app.get("/list_buckets", async (req: Request, res: Response): Promise<void> => {
  try {
...
app.get("/list_files/:bucketName", async (req: Request, res: Response): Promise<void> => {
...
app.get("/list_all_user_files", async (req: Request, res: Response): Promise<void> => {
  const fileList = [];
  try {
...
app.get("/list_user_files/:userDID", async (req: Request, res: Response): Promise<void> => {
  const { userDID } = req.params;
...
app.get("/list_public_files", async (req: Request, res: Response): Promise<void> => {
  const fileList = [];
  try {
...
app.get("/list_public_manifests", async (req: Request, res: Response): Promise<void> => {
  const fileList = [];
  try {
    ...
    logger.error("Error listing public manifests: " + (error as Error).message);
...
app.get("/get_user_public_files/:username", async (req: Request, res: Response): Promise<void> => {
    const { username } = req.params;
    const prefix = `public/${username}/`;
    const fileList = [];
    ...
    try {
      ...
      logger.error("Error listing user public files: " + (error as Error).message);
      res.status(500).json({ error: "Error listing user public files" });
      return;
    }
});

app.get("/list_user_manifests/:userDID", async (req: Request, res: Response): Promise<void> => {
  const { userDID } = req.params;
  ...
  try {
    ...
    logger.error("Error listing user manifests: " + (error as Error).message);
...
// save manifest draft
app.post("/save_user_manifest_draft", async (req: Request, res: Response): Promise<void> => {
...
app.get("/get_stored_content_and_manifest_draft/:userDID/:uploadId", async (req: Request, res: Response): Promise<void> => {
...
app.get("/list_signed_user_files/:userDID", async (req: Request, res: Response): Promise<void> => {
  const { userDID } = req.params;
  ...
  try {
    ...
    logger.error("Error listing signed user files: " + (error as Error).message);
    res.status(500).json({ error: "Error listing signed user files" });
    return;
  }
});

app.get("/get_signed_user_file/:userDID/:fileId", async (req: Request, res: Response): Promise<void> => {
  const { userDID, fileId } = req.params;
  const prefix = `signed-users/${userDID}/`;
  try {
...
app.post("/get_file_by_mnemonic", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    fileMapStream.on("error", (err: any) => {
...
app.post("/get_public_file_by_mnemonic", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    logger.error("Error getting public file by mnemonic: " + (error as Error).message);
...
app.post("/publish-private-file", uploadLimiter as any, async (req: Request, res: Response): Promise<void> => {
  const {
    fileId,
    mnemonicId,
    userDID,
  ...
  try {
    ...
    await minioClient.putObject(
      ...
      publicPath,
      fileBuffer,
      privateFileStat.size,
      { "Content-Type": privateFileStat.metaData["content-type"] || "application/octet-stream" }
    );
    ...
    await minioClient.putObject(
      BUCKET,
      publicFileMapPath,
      Buffer.from(JSON.stringify(publicFileMapData)),
      undefined,
      { "Content-Type": "application/json" }
    );
    ...
    logger.error("Error publishing file: " + (error as Error).message);
...
app.post("/webhooks/signing-completed", async (req: Request, res: Response): Promise<void> => {
  try {
